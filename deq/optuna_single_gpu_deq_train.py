...
def make_objective(train_dl, test_dl, *, n_epochs: int, batch_size: int, chan: int = N_CHANNELS):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        nudging = trial.suggest_float("nudging", 0.01, 0.1, log=True)
        lr_w = trial.suggest_float("lr_w", 0.00025, 0.0025, log=True)
        wd_w = trial.suggest_float("wd_w", 0.005, 0.1, log=True)
        lr_h = trial.suggest_float("lr_h", 0.1, 0.5, log=True)
        mom_h = trial.suggest_float("mom_h", 0.7, 2.0)
        init_scale = trial.suggest_float("init_scale", 0.000005, 0.0005, log=True)
...
def main():
    ...
    study_name = "deqpc_cifar10_smallest_init"
