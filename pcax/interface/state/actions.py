...
class SACreateDefaultMasks(StateAction):
    ...
    def __call__(self, state: _State, model: Any, param_filter: Callable[[Any], bool] = is_param, **kwargs) -> None:
        ...
        def mask_fn(node, state, is_leaf):
            ...
            if is_leaf and param_filter is not None and param_filter(node, state) is False:
                state = None

            return state

        type_info_mask = state.create_mask(model, mask_fn, root=NodeInfo())
        state.save_mask("type", jtu.tree_map(lambda node_info: node_info.type, type_info_mask))
        state.save_mask("status", jtu.tree_map(lambda node_info: node_info.status, type_info_mask))
...
class SAStandardiseModelParameters(StateAction):
    ...
    def __call__(self, state: _State, model: Any, **kwargs):
        ...
        def where(model):
            return jtu.tree_leaves(jtu.tree_map(lambda _, value: value, *state.get_masks("type"), model))
    ...
    def __call__(
        ...
        **kwargs
        ...
        if trainer is not None:
            model, _ = trainer.init_fn(state, model, jnp.empty(shape=(batch_size, *input_shape)), *init_fn_args)

        r_kwargs = {"model": model}

        if optim_fn is not None:
            assert trainer is not None, "Trainer must be provided to create the optimizer"
            # Create optimizer now that both neurons and weights are initialized with the correct shape
            optim = optim_fn(state)
            optim_mask = optim.init([state.map_mask(lambda _, param: param, ["type", model])])
