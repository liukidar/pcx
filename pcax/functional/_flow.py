...
# ```
#
# Instead, we rely on the primitives in jax.lax (i.e., cond, scan, ...), which are wrapped by pcax to allow for automatic
# parameter tracking. Each transformation is a thin wrapper around the corresponding jax.lax function and behaves 
# analogously to its jax counterpart. The main difference is that the input and output arguments require a slighlty
# different structure to simplify usage. Check the documentation of each transformation for more details.
# Furthermore, to be consistent with the _BaseTransformation class, each transformation accepts the function
# arguments via the __call__ method and not directly in the constructor (i.e., Scan(f, ...)(x), instead of scan(f, x, ...)
# as it is in jax). However, the utility functions provided at the end of this file allow to use the jax syntax as well.
#
########################################################################################################################
...
class Switch(_BaseTransform):
    ...
    def __init__(
        self,
        fns: Sequence[_BaseTransform | Callable],
        **t_kwargs: Any
    ):
        """Switch constructor.
