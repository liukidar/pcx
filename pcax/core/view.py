...
class View:
	...
	def get(self, root:jnp.ndarray, _apply_t:bool = False, _cache:bool = False) -> jnp.ndarray:
		...
		if _apply_t and self.transformation_fn is not None:
	...
	def match(self, name:str, type:str=None, _only_leaves:bool=True) -> 'TmpView':
		...
		if name[0] == self.name or name[0] == '*':
			if len(name) > 1:
				next_name = name[1]
			elif _only_leaves == False or len(self.children) == 0 and (type is None or isinstance(self, name2type[type])):
				r.append(self)
...
class OutputView(View):
	...
	def set(self, root:jnp.ndarray, _x:jnp.ndarray, _cache:bool = None):		
		self.cached = self.transformation_fn(_x) if self.transformation_fn is not None else _x

	def get(self, root:jnp.ndarray, _cat_mode:str='cat', _cache:bool = True, **_kwargs) -> jnp.ndarray|Tuple[jnp.ndarray]:
		if _cache == True and self.cached is not None:
			return self.cached
...
class TmpView(View):
	...
	def get(self, *args, **kwargs) -> jnp.ndarray:
		return tuple(map(lambda root_view: root_view[1].get(root_view[0], *args, **kwargs), self.children))
