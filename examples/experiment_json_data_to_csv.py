...
for folder in os.listdir(experiments_path):
    ...
    if os.path.isdir(folder_path) and any(graph in folder for graph in graph_types):
        ...
        # Extract the last values of MAEs, SHDs, and energies
        last_mae = metrics['MAEs'][-1] if metrics['MAEs'] else None
        last_shd = metrics['SHDs'][-1] if metrics['SHDs'] else None
        last_energy = metrics['energies'][-1] if metrics['energies'] else None
