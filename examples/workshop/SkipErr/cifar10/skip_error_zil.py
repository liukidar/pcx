...
class SkipError(pxc.EnergyModule):
    def __init__(
        ...
        self.backward_layers = [pxnn.Linear(num_classes, hidden_dim, bias=False) for i in range(len(backward_indices))]
    ...
    def __call__(self, x, y=None):
        ...
        if y is not None:
            ...
            for i, index in enumerate(self.backward_indices):
                skip_error = self.backward_layers[i](error)
                current_state = self.vodes[index].get("h")
                self.vodes[index].set("h", current_state - self.beta * (current_state - skip_error))
...
def train_on_batch(
    ...
    for t in range(T):
        ...
        for i in range(0, len(model.vodes) - t - 2):
            mask.vodes[i].h = False
        with pxu.step(model, clear_params=pxc.VodeParam.Cache):
            _, g = pxf.value_and_grad({"model": mask}, has_aux=True)(energy)(x, model=model)
    ...
    with pxu.step(model, clear_params=pxc.VodeParam.Cache):
        ...
        for i, index in enumerate(model.backward_indices):
            g["model"].backward_layers[i].nn.weight.set(
                -jnp.mean(
                    jnp.expand_dims(model.vodes[index].get("h"), -1)
                    @ jnp.transpose(
                        jnp.expand_dims(model.vodes[-1].get("h") - model.vodes[-1].get("u"), -1), (0, 2, 1)
                    ),
                    axis=0,
                )
            )
...
def run_experiment(
    *,
    ...
    model = SkipError(
        ...
        beta=beta,
        backward_indices=backward_indicies,
    )
    ...
    optim_h = pxu.OptimTree(
        optax.sgd(learning_rate=optax.linear_schedule(optim_x_lr, 0.05, 1), momentum=optim_x_momentum),
        leaf_fn=lambda x: isinstance(x, pxc.Vode),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/skiperror_cifar10_adamw_hypertune_zil.yaml",
    ...
    run_experiment(
        ...
        backward_indicies=get_config_value(config, "backward_indices"),
