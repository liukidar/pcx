...
def train_on_batch(
    ...
    for t in range(L):
        with pxu.step(model, clear_params=pxc.VodeParam.Cache):
            ...
            mask.layers[L - t - 1] = pxu.M(pxnn.LayerParam).to([False, True])(model.layers[L - t - 1], is_pytree=True)

            (e, y_), g = pxf.value_and_grad({"model": mask}, has_aux=True)(energy)(x, model=model)
...
def run_experiment(
    *,
    ...
    optim_h = pxu.OptimTree(
        optax.sgd(learning_rate=optim_x_lr, momentum=optim_x_momentum),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/skiperror_cifar10_adamw_hypertune_bp.yaml",
