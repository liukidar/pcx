...
class VGGNet_skip(pxc.EnergyModule):
    def __init__(
        ...
        #self.vgg_type = config.model.vgg_type
        self.feature_layers, vodes_feature, self.long_skip = self.init_convs(VGG_types["VGG19"], in_channels, in_height, in_width)
    ...
    def __call__(self, x: jax.Array, y: jax.Array):
        for ind, (block, node) in enumerate(zip(self.feature_layers, self.vodes[: len(self.feature_layers)])):
            ...
            if ind == 2:
                long_skip_value = self.long_skip(x.flatten())
        x = x.flatten()
    ...
    def init_convs(self, architecture, in_channels, in_height, in_width):
        ...
        for i in range(len(architecture) - 1):
            ...
            if type(x) == int:
                ...
                in_channels = x
                if i == 4:
                    # long_skip = pxnn.Linear(out_channel * in_height * in_width, self.nm_classes.get())
                   long_skip = pxnn.Linear(512 * 8 * 8, self.nm_classes.get())  # Hardcoded for VGG19

                    #long_skip = pxnn.Linear(out_channel * 8 * 8, self.nm_classes.get())  # Hardcoded for VGG19
            else:
                pass
        return layers, vodes, long_skip
    ...
    def init_fcs(self, architecture, in_height, in_width, num_hidden, nm_classes):
        print('artchitecture:', architecture)
...
def forward(x, y, *, model: VGGNet_skip):
    return model(x, y)


@pxf.vmap(pxu.Mask(pxc.VodeParam | pxc.VodeParam.Cache, (None, 0)), in_axes=(0,), out_axes=(None, 0), axis_name="batch")
def energy(x, *, model: VGGNet_skip):
    y_ = model(x, None)
    return jax.lax.pmean(model.energy().sum(), "batch"), y_

@pxf.jit(static_argnums=[0,6])
def train_on_batch(T: int, x: jax.Array, y: jax.Array, *, model: VGGNet_skip, optim_w: pxu.Optim, optim_h: pxu.Optim):
    model.train()
    optim_h.init(model)
...
@pxf.jit()
def eval_on_batch(x: jax.Array, y: jax.Array, *, model: VGGNet_skip):
    model.eval()
...
def train(dl, T, *, model: VGGNet_skip, optim_w: pxu.Optim, optim_h: pxu.Optim):
...
def main(run_info: stune.RunInfo):
    # print run_info
    intermidiate_savepath = run_info["study"] + '.json'
