...
# own
from utils import (
    create_new_test_loader_with_batch_size,
    get_dataloaders,
    set_random_seed,
    DatasetProcessorData,
)
...
class Model(pxc.EnergyModule):
    def __init__(
        ...
        self.vodes = [pxc.Vode((hidden_dim,)) for _ in range(nm_layers - 1)] + [
...
def train(dl, T, *, model: Model, optim_w: pxu.Optim, optim_h: pxu.Optim):
    for x, y in dl:
        train_on_batch(
            T, x, jax.nn.one_hot(y, 10), model=model, optim_w=optim_w, optim_h=optim_h
        )
...
@pxf.jit(static_argnums=0)
def eval_inference_on_batch(
    T: int, x: jax.Array, y: jax.Array, *, model: Model, optim_h: pxu.Optim
):
    model.eval()
...
def eval_inference(dl, T, *, model: Model, optim_h: pxu.Optim):
    for x, y in dl:
        eval_inference_on_batch(
            T, x, jax.nn.one_hot(y, 10), model=model, optim_h=optim_h
        )
...
MNIST_dataset = get_dataloaders(
    MNIST_dataset_name, train_subset_size, batch_size, noise_level
)
...
FMNIST_dataset = get_dataloaders(
    FMNIST_dataset_name, train_subset_size, batch_size, noise_level
)
# Check the sizes of the datasets
...
EMNIST_dataset = get_dataloaders(
    EMNIST_dataset_name, train_subset_size, batch_size, noise_level
)
# Check the sizes of the datasets
...
KMNIST_dataset = get_dataloaders(
    KMNIST_dataset_name, train_subset_size, batch_size, noise_level
)
# Check the sizes of the datasets
...
notMNIST_dataset = get_dataloaders(
    notMNIST_dataset_name, train_subset_size, batch_size, noise_level
)
# Check the sizes of the datasets
...
test_set_sizes = [
    len(MNIST_dataset.test_loader.dataset),
    len(FMNIST_dataset.test_loader.dataset),
    len(EMNIST_dataset.test_loader.dataset),
    len(KMNIST_dataset.test_loader.dataset),
    len(notMNIST_dataset.test_loader.dataset),
]
...
model = Model(
    input_dim=784, hidden_dim=512, output_dim=10, nm_layers=3, act_fn=jax.nn.gelu
)
...
with pxu.step(model, pxc.STATUS.INIT, clear_params=pxc.VodeParam.Cache):
    ...
    optim_w = pxu.Optim(
        optax.sgd(w_learning_rate, momentum=0.95), pxu.Mask(pxnn.LayerParam)(model)
    )
...
for e in range(nm_epochs):
    # train the model
    train(
        MNIST_dataset.train_loader, T=T, model=model, optim_w=optim_w, optim_h=optim_h
    )
    ...
    print(
        f"Epoch {e+1}/{nm_epochs} - Train Acc: {a_train:.4f} - Val Acc: {a_val:.4f} - Train Loss: {e_train:.4f} - Val Loss: {e_val:.4f}"
    )
...
class DatasetProcessor:
    def __init__(
        self, model, dataset, dataset_name, new_batch_size, T_inf=100, optim_h=None
    ):
        self.model = model
    ...
    def predict_with_logits(self):
        ys_test, logits_test = predict_with_logits(
            self.full_test_loader, model=self.model
        )
        self.data.ys_test_jax = jnp.array(ys_test)
        self.data.logits_test_jax = jnp.array(logits_test)
        self.data.max_softmax_values = self.max_softmax_per_sample(
            self.data.logits_test_jax
        )
