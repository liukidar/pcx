...
class Complete_Graph(pxc.EnergyModule):
    def __init__(self, input_dim: int, n_nodes: int, has_bias: bool = False) -> None:
        ...
        # Initialize a single linear layer for the weights
        self.layers = [pxnn.Linear(n_nodes, n_nodes, bias=has_bias)]
        
        # Zero out diagonal weights to avoid self-loops
        weight_matrix = self.layers[0].nn.weight.get()
        weight_matrix = jax.numpy.fill_diagonal(weight_matrix, 0.0, inplace=False)
        self.layers[0].nn.weight.set(weight_matrix)
    ...
    def get_W(self):
        ...
        return self.layers[0].nn.weight.get()
    ...
    def __call__(self, x=None):
        ...
        if x is not None:
            ...
            # Get current node values and reshape for linear layer
            x_ = self.vodes[0].get('h')
            x_ = x_.reshape(n_nodes, input_dim)
            
            # Apply linear transformation
            output = self.layers[0](x_.squeeze(-1))
