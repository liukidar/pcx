...
class Module(NodeModule):
	...
	def __call__(self, mu:Any, output_view:str|List[str]=None, input_view:str|List[str]=None, view:str|List[str]=None) -> View:
		assert view is None or (output_view is None and input_view is None), "You cannot set both 'view' and 'output/input_view' at the same time."
	...
	def __get_submodules(self):
		return (getattr(self, field.name) for field in dataclasses.fields(self) if issubclass(field.type, Module))
...
class Layer(Module):
	...
	def __init__(self,
		...
		if view is not None:
			...
			self.view = View(_children = [
				InputView(),
				OutputView(_energy_fn=lambda root, x, mu: gaussian_energy(mu.get(root), x.get(root)))
			])
	...
	def energy(self, _criterion) -> jnp.array:
		...
		return jtu.tree_reduce(lambda e1, e2: e1 + e2, list(map(lambda view: _criterion(view), energy_views)))
